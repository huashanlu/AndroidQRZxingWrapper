...
package com.hisun.qrcode.scanner.camera;
...
import com.hisun.qrcode.scanner.common.Config;
...
//		doSetTorch(parameters, false);
	...
	void setTorch(Camera camera, boolean newSetting) { // true打开
		Camera.Parameters parameters = camera.getParameters();
...
//		boolean currentSetting = ECQrCodeManager.getInstance(context).getLightMode() == FrontLightMode.ON;
...
//		 }
		...
		FrontLightMode lightMode = ECQrCodeManager.getInstance(context)
				.getLightMode();
		...
		if (lightMode != null) {
			...
			} else if (lightMode == FrontLightMode.ON) {
			...
			} else if (lightMode == FrontLightMode.OFF) {
				flashMode = findSettableValue(
						parameters.getSupportedFlashModes(),
						Camera.Parameters.FLASH_MODE_OFF);
